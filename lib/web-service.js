...
const tokenFromConfigObject = (githubTokenConfig) => {
  ...
  if (
    tokenFileEnvVar !== undefined &&
    typeof tokenFileEnvVar === 'string' &&
    tokenFileEnvVar.length > 0
  ) {
    ...
    if (tokenFile !== undefined && tokenFile.length > 0) {
      if (fs.existsSync(tokenFile)) {
        ...
        debug(
          `Get github token: File defined in environment variable '${tokenFileEnvVar}' ('${tokenFile}') does not exist`,
        );
      }
    }
  } else {
    ...
    if (
      ...
      typeof tokenEnvVar === 'string' &&
      tokenEnvVar.length > 0
    ) {
      // eslint-disable-next-line security/detect-object-injection
  ...
  const gotOptions = {
    ...
    hooks: {
      beforeRetry: [
        ...
        (error, retryCount) => {
          debugForApp(
            `http request to github api for package "${name}" license info failed, retrying again soon...`,
          );
        },
      ],
    },
 ...
 * @returns {object} object with the options for 'got' request
 */
const gotGithubLicenseFileOptions = (
  maxAttempts,
  url,
  githubTokenConfig = {},
) => {
  const gotOptions = {
    ...
    hooks: {
      beforeRetry: [
        ...
        (error, retryCount) => {
          debugForApp(
            `http request to github for license file at "${url}" failed, retrying again soon...`,
          );
        },
      ],
      beforeError: [
 ...
 * @param {object[]} packagesInfos - array of objects generated from the license-report generated json file
 ...
 */
export const addLicenseFilePath = async (
  packagesInfos,
  httpRetryOptions = {},
  authorizationOptions = {},
) => {
  if (
    packagesInfos !== undefined &&
    Array.isArray(packagesInfos) &&
    packagesInfos.length > 0
  ) {
    await Promise.all(
      packagesInfos.map(async (element) => {
        ...
        if (element.link !== undefined) {
          ...
          if (githubPackageName.length !== 0) {
            ...
            let options = {};
            try {
              options = gotGithubJsonOptions(
                httpRetryOptions.maxAttempts,
                githubPackageName,
                authorizationOptions,
              );
              ...
              if (
                licenseInfo.download_url !== undefined &&
                licenseInfo.download_url.length > 0
              ) {
              ...
              if (response?.statusCode === 403) {
                ...
                if (headers['x-ratelimit-remaining'] === '0') {
                  const timeForRateLimitReset =
                    parseInt(error.response.headers['x-ratelimit-reset']) -
                    Math.floor(new Date().getTime() / 1000);
                  const resetMsg = timeForRateLimitReset
                    ? ` please wait ${timeForRateLimitReset} seconds before trying again.`
                    : '';
                  console.error(
                    `${githubPackageName}: License query failed. Rate limit of ${headers['x-ratelimit-limit']} requests per hour exceeded.${resetMsg}`,
                  );
                  if (!options.headers?.Authorization)
                    console.error(
                      'You can increase the rate limit by providing a github token. See the README for more information.',
                    );
                  debugForApp(
                    `Failed query '${licenseInfoAdr}' ('${error.message}')`,
                  );
                  return;
                }
              }
              console.error(
                `the project '${githubPackageName}' does not contain a license file on github; request returned ${response.statusCode} ( ${response.statusMessage})`,
              );
              debugForApp(
                `no license file on '${licenseInfoAdr}' ('${error.message}')`,
              );
            }
          }
        }
 ...
 * @param {object[]} packagesInfos - array of objects with package infos including path to license file
 * @param {string} targetDirectory - path of directory for downloaded license files
 * @param {object} httpRetryOptions - http retry options for 'got' requests ({maxAttempts})
 * @param {object} authorizationOptions - authorization options for 'got' requests {tokenEnvVar, tokenFileEnvVar}
 */
export const downloadLicenseFiles = async (
  packagesInfos,
  targetDirectory,
  httpRetryOptions = {},
  authorizationOptions = {},
...
export default {
  ...
  getPackageNameFromLink, // required to run tests on this internal function
