    ...
    await Promise.all(packagesInfos.map(async element => {
      ...
      if (element.link !== undefined) {
        ...
        if (githubPackageName.length !== 0) {
          ...
          let options = {};
          try {
            options = gotGithubJsonOptions(httpRetryOptions.maxAttempts, githubPackageName, authorizationOptions);
            ...
            if (response?.statusCode === 403) {
              ...
              if (headers['x-ratelimit-remaining'] === '0') {
                const timeForRateLimitReset = parseInt(error.response.headers['x-ratelimit-reset']) - Math.floor(new Date().getTime() / 1000);
                const resetMsg = timeForRateLimitReset ? ` please wait ${timeForRateLimitReset} seconds before trying again.` : '';
                console.error(`${githubPackageName}: License query failed. Rate limit of ${headers['x-ratelimit-limit']} requests per hour exceeded.${resetMsg}`);
                if (!options.headers?.Authorization)
                  console.error('You can increase the rate limit by providing a github token. See the README for more information.');
