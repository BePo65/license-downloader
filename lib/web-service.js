 ...
 */
function gotGithubJsonOptions(maxAttempts, name, githubTokenConfig = {}) {
  const gotOptions = {
    responseType: 'json',
    retry: maxAttempts,
    hooks: {
  ...
  const token = util.tokenFromConfigObject(githubTokenConfig);
  /* eslint-disable security/detect-possible-timing-attacks */
  /* eslint-disable security-node/detect-possible-timing-attacks */
  if (token !== undefined) {
    gotOptions.headers = {
      Authorization: `${token}`
    };
  ...
  /* eslint-enable security-node/detect-possible-timing-attacks */
 ...
 * @returns {object} object with the options for 'got' request
 */
function gotGithubLicensefileOptions(maxAttempts, url, githubTokenConfig = {}) {
  const gotOptions = {
    retry: maxAttempts,
    hooks: {
      beforeRetry: [
 ...
 */
export async function addLicenseFilePath(packagesInfos, httpRetryOptions = {}, authorizationOptions = {}) {
 ...
 * @param {object} authorizationOptions - authorization options for 'got' requests {tokenEnvVar, tokenFileEnvVar}
 */
export async function downloadLicenseFiles(packagesInfos, targetDirectory, httpRetryOptions = {}, authorizationOptions = {}) {
  ...
  for (const packageInfos of packagesInfos) {
    if (packageInfos.licenseFileLink.length > 0) {
      const { scope, packageName } = util.licenseFileName(packageInfos.name);
